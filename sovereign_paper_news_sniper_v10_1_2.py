"""
Sovereign Paper News Sniper v10.1.2-ELITE PRODUCTION
TUNED: OBI signal loosened, stop loss wider, overtrading reduced, hold time extended
Changes from v10.1.1:
  [TUNE-1] OBI thresholds loosened: buy=0.58->0.52, sell=0.42->0.48 — OBI was silent
  [TUNE-2] min_stacked_edge lowered: 0.80->0.42 — was rejecting almost everything
  [TUNE-3] min_single_signal_score lowered: 0.22->0.12 — allow weaker individual signals
  [TUNE-4] stop_loss_pct widened: 0.03->0.05 — 68% stop-outs means SL too tight
  [TUNE-5] max_hold_seconds increased: 1800->3600 — give trades more time to work
  [TUNE-6] take_profit_pct raised: 0.05->0.08 — avg win was tiny ($0.38)
  [TUNE-7] max_trades_per_loop reduced: 10->4 — 619 trades on 10k ticks = overtrading
  [TUNE-8] loop_sleep increased: 12->20 — slow down cycle rate
  [TUNE-9] edge_decay_tau increased: 45->120 — signals decaying too fast
  [TUNE-10] trailing_stop_activation raised: 0.025->0.05 — arm trailing stop later
  [TUNE-11] OBI depth_range widened: 0.04->0.08 — capture more book depth
  [TUNE-12] momentum_min_move loosened: 0.0018->0.0012 — MOM needs less price move
  [TUNE-13] mean_rev_threshold loosened: 0.0022->0.0015 — REVT fires on smaller deviations
"""
...
VERSION = "v10.1.2-ELITE | TUNED | DEEP-NEWS-FIXED | 28-INFRA | ELITE-EXIT"
MODE = "PAPER"
@dataclass
class RiskConfig:
...
class SignalConfig:
    ...
    min_trade_score: float = 0.20
    min_signal_relevance: float = 0.25
    min_soft_relevance: float = 0.08
    min_effective_edge: float = 0.025
    ...
    edge_gate_min_momentum: float = 0.0008
    edge_gate_min_dislocation: float = 0.0050
    edge_gate_min_edge_score: float = 0.28
    ...
    p24_min_signal_score: float = 0.32
...
@dataclass
class ExitConfig:
    """Elite exit engine — [TUNE-4,5,6,9,10] wider SL, longer hold, bigger TP"""
    take_profit_pct: float = 0.08        # [TUNE-6] raised from 0.05
    stop_loss_pct: float = 0.05          # [TUNE-4] raised from 0.03 (68% SL rate fix)
    max_hold_seconds: int = 3600         # [TUNE-5] raised from 1800
    min_hold_seconds: int = 30
    trailing_stop_activation: float = 0.05   # [TUNE-10] raised from 0.025
    trailing_stop_distance: float = 0.020
    tier1_tp_pct: float = 0.03
    tier2_tp_pct: float = 0.06
    tier3_tp_pct: float = 0.10
    trailing_tp_enabled: bool = True
    trailing_tp_ratchet_pct: float = 0.60
    trailing_tp_min_gain: float = 0.03   # raised from 0.02
    edge_decay_exit_enabled: bool = True
    edge_decay_exit_threshold: float = 0.008
    edge_decay_tau: float = 120.0        # [TUNE-9] raised from 45.0 — signals decayed too fast
    book_deterioration_exit: bool = True
    book_deterioration_depth_pct: float = 0.25
    fast_fail_exit_pct: float = 0.18
    stier_low_conf_stop_pct: float = 0.05
    stier_high_conf_stop_pct: float = 0.08
    stier_high_conf_threshold: float = 0.70
    obi_exit_tp_mult: float = 0.9
    obi_exit_sl_mult: float = 1.0
    mom_exit_tp_mult: float = 1.2
    mom_exit_sl_mult: float = 0.9
    revt_exit_tp_mult: float = 1.0
    revt_exit_sl_mult: float = 1.2
    pre_event_tp_mult: float = 0.8
    pre_event_sl_mult: float = 0.8
    thin_book_tp_mult: float = 0.7
    thin_book_sl_mult: float = 0.6
    high_vol_tp_mult: float = 1.3
    high_vol_sl_mult: float = 1.5
    trend_tp_mult: float = 1.4
    trend_sl_mult: float = 0.8
@dataclass
class PriceSignalConfig:
    """[TUNE-1,11,12,13] OBI, MOM, REVT all loosened"""
    min_signal_score: float = 0.12       # loosened from 0.16
    confluence_min_score: float = 0.16
    max_signal_markets: int = 250
    obi_buy_threshold: float = 0.52      # [TUNE-1] loosened from 0.54 — OBI was silent
    obi_sell_threshold: float = 0.48     # [TUNE-1] loosened from 0.46
    obi_depth_range: float = 0.08        # [TUNE-11] widened from 0.04 — capture more book
    obi_depth_levels: int = 5
    momentum_min_ticks: int = 1
    momentum_min_move: float = 0.0012    # [TUNE-12] loosened from 0.0018
    mean_rev_threshold: float = 0.0015   # [TUNE-13] loosened from 0.0022
    mean_rev_min_ticks: int = 2
    arb_threshold: float = 0.010
    price_history_maxlen: int = 60       # more history for better signals
@dataclass
class EdgeStackingConfig:
    """[TUNE-2,3] min_stacked_edge lowered dramatically, single signal score loosened"""
    obi_weight: float = 0.45             # boosted OBI weight
    momentum_weight: float = 0.30
    reversion_weight: float = 0.25
    spread_weight: float = 0.20
    news_weight: float = 0.20            # boosted news weight
    dshift_weight: float = 0.15
    min_stacked_edge: float = 0.42       # [TUNE-2] lowered from 0.80 — was blocking everything
    min_agreeing_signals: int = 1
    confluence_bonus: float = 0.12
    min_single_signal_score: float = 0.12  # [TUNE-3] lowered from 0.22
...
class ExecutionPriorityConfig:
    ...
    tier1_min_edge: float = 0.08
    tier2_min_edge: float = 0.05
    tier3_min_edge: float = 0.03
...
class AdaptiveThresholdsConfig:
    ...
    high_vol_edge_multiplier: float = 1.3   # reduced from 1.5 — was too restrictive
    low_vol_edge_multiplier: float = 0.8
    wide_spread_threshold: float = 0.05
    tight_spread_threshold: float = 0.01
    wide_spread_confidence_boost: float = 0.15
    low_liquidity_threshold: float = 1500.0
    high_liquidity_threshold: float = 50000.0
    low_liq_confidence_multiplier: float = 1.1
@dataclass
class FeedbackReinforcementConfig:
    enabled: bool = True
    min_trades_per_signal: int = 8
...
class MarketExpansionConfig:
    min_markets_per_cycle: int = 80
    max_markets_per_cycle: int = 300
    rotation_pct_per_cycle: float = 0.30
...
class MarketConfig:
    ...
    min_effective_edge: float = 0.025
    max_entry_premium_pct: float = 0.180
    max_chase_move: float = 0.025
    market_cooldown: int = 45
    min_market_liquidity: float = 300.0
    min_market_volume: float = 30.0
    max_market_duration_days: int = 200
    execution_quality_enabled: bool = True
    execution_quality_floor: float = 0.06   # loosened from 0.08
    entry_fallback_spread: float = 0.050
    min_best_bid: float = 0.005
    max_best_ask: float = 0.995
    min_top_level_size: float = 3.0
    min_combined_depth: float = 20.0        # loosened from 25
@dataclass
class MarketScoringConfig:
    ...
    age_half_life_hours: float = 72.0
@dataclass
class ExecutionEVConfig:
    min_ev_threshold: float = 0.003        # loosened from 0.005
    fill_prob_sigmoid_center: float = 0.05
    fill_prob_sigmoid_steepness: float = 5.0  # gentler from 6.0
    fill_prob_depth_scale: float = 120.0   # loosened from 150
    slippage_base: float = 0.002
    slippage_depth_factor: float = 0.5
    slippage_alpha: float = 1.35
@dataclass
class CacheConfig:
...
@dataclass
class LoopConfig:
    """[TUNE-7,8] Reduced trade frequency to stop overtrading"""
    loop_sleep: int = 20                 # [TUNE-8] raised from 12
    request_timeout: float = 15.0
    no_trade_sleep: int = 20
    max_trades_per_loop: int = 4         # [TUNE-7] reduced from 10
    max_markets_scan: int = 2000
    max_ranked_markets: int = 500
    market_fetch_window: int = 800
    max_llm_concurrency: int = 6
    max_book_concurrency: int = 64
    discovery_exploration_ratio: float = 0.28
    market_discovery_min_rotation: int = 5
    market_pool_refresh_cycles: int = 15
    market_pool_ttl: int = 500
    market_sort_rotation: List[str] = field(default_factory=lambda: [
        "volume", "liquidity", "newest", "ending_soon", "spread"])
...
class PaperTradingConfig:
    ...
    signal_decay_ttl_seconds: float = 60.0  # raised from 25
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        c.exit.take_profit_pct = float(os.getenv("TAKE_PROFIT_PCT", "0.08"))
        c.exit.stop_loss_pct = float(os.getenv("STOP_LOSS_PCT", "0.05"))
        c.exit.max_hold_seconds = int(os.getenv("MAX_HOLD_SECONDS", "3600"))
        c.exit.edge_decay_tau = float(os.getenv("EDGE_DECAY_TAU", "120.0"))
        c.exit.trailing_stop_activation = float(os.getenv("TRAILING_STOP_ACTIVATION", "0.05"))
        c.edge_stacking.min_stacked_edge = float(os.getenv("MIN_STACKED_EDGE", "0.42"))
        c.edge_stacking.min_agreeing_signals = int(os.getenv("MIN_AGREEING_SIGNALS", "1"))
        c.edge_stacking.min_single_signal_score = float(os.getenv("MIN_SINGLE_SIGNAL_SCORE", "0.12"))
        c.loop.loop_sleep = int(os.getenv("LOOP_SLEEP", "20"))
        ...
        c.loop.max_trades_per_loop = int(os.getenv("MAX_TRADES_PER_LOOP", "4"))
        ...
        c.market.execution_quality_floor = float(os.getenv("EXECUTION_QUALITY_FLOOR", "0.06"))
        ...
        c.market.min_combined_depth = float(os.getenv("MIN_COMBINED_DEPTH", "20"))
        c.market.max_entry_premium_pct = float(os.getenv("MAX_ENTRY_PREMIUM_PCT", "0.18"))
        c.price_signal.obi_buy_threshold = float(os.getenv("OBI_BUY_THRESHOLD", "0.52"))
        c.price_signal.obi_sell_threshold = float(os.getenv("OBI_SELL_THRESHOLD", "0.48"))
        c.price_signal.obi_depth_range = float(os.getenv("OBI_DEPTH_RANGE", "0.08"))
        c.price_signal.momentum_min_move = float(os.getenv("MOMENTUM_MIN_MOVE", "0.0012"))
        c.price_signal.mean_rev_threshold = float(os.getenv("MEAN_REV_THRESHOLD", "0.0015"))
        ...
        c.execution_ev.min_ev_threshold = float(os.getenv("MIN_EV_THRESHOLD", "0.003"))
        return c
...
class StackedEdge:
    ...
    created_at: float = field(default_factory=time.time); adaptive_threshold: float = 0.42
    @property
...
class PrioritizedOpportunity:
    ...
    def __lt__(self, other):
        ...
        return self.execution_ev > other.execution_ev
@dataclass
...
class TradeAnalysis:
    ...
    effective_sl_pct: float = 0.05; auto_direction: bool = False; allocator_score: float = 0.0
    stacked_edge_score: float = 0.0; stacked_edge_obj: Optional[StackedEdge] = None
    tier: ExecutionTier = ExecutionTier.TIER3_SLOW; adaptive_threshold: float = 0.42
...
class Position:
    ...
    trailing_armed: bool = False; bucket: str = "general"; effective_sl_pct: float = 0.05
...
class StructuredLogger:
    ...
    @staticmethod
    def entry_attempt(cycle, market, direction, edge, score, ev, spread, depth, signals, regime, size):
    ...
    @staticmethod
    def entry_reject(cycle, market, direction, reason, edge=0, score=0, ev=0, spread=0, depth=0, signals=""):
        StructuredLogger._emit({
    ...
    @staticmethod
    def entry_filled(cycle, market, direction, fill_price, size, ev, signals, slippage_bps=0):
        StructuredLogger._emit({
    ...
    @staticmethod
    def exit_triggered(tid, reason, pnl, pnl_pct, hold_seconds, regime=""):
        StructuredLogger._emit({
...
def regime_adjusted_edge_threshold(base: float, regime: str) -> float:
    mults = {"high_vol": 1.20, "trend": 0.90, "mean_revert": 1.00}
    return base * mults.get(regime, 1.0)
def decayed_edge(edge: float, seconds_since_signal: float, tau: float = 120.0) -> float:
    return edge * math.exp(-seconds_since_signal / tau)
def should_delay_entry(spread: float, recent_spreads: List[float]) -> bool:
    if not recent_spreads: return spread > 0.10
    avg = sum(recent_spreads) / len(recent_spreads)
    return spread > max(0.10, avg * 1.5)
...
def dynamic_execution_thresholds(regime: str) -> Dict[str, float]:
    ...
    return {"min_best_bid": 0.005, "min_depth": 20, "max_premium": 0.10}
...
class EliteExitEngine:
    ...
    def check_exit(self, pos: Position, current_price: float, pnl_pct: float,
        ...
        if current_signal == pos.side: mh *= 1.3
        if regime == "trend" and pnl_pct > 0: mh *= 1.5
        if ht >= mh:
...
def _load_v28_infra_module():
    ...
    for candidate in candidates:
        ...
        try:
            ...
            logger.info(f"
 Loaded v28 infrastructure from {candidate.name}"); return mod
        except Exception as e: logger.warning(f"
 Failed loading v28 infra from {candidate.name}: {e}")
    return None
...
class PriceSignalEngine:
    ...
    def _momentum(self, tid):
        ...
        if cons < 0.60: return None  # relaxed from 0.70
    ...
    def _depth_shift(self, tid, bids_raw, asks_raw):
        ...
        bc = (cbd - ab) / max(ab, 100); ac = (cad - aa) / max(aa, 100); th = 0.40  # lowered from 0.50
        if bc > th and bc > ac:
...
class EdgeStackingEngine:
    ...
    def stack_signals(self, obi_signal=None, mom_signal=None, revt_signal=None,
        ...
        for name, signal, weight in signal_data:
            ...
            ttl = {"OBI": 8.0, "MOM": 20.0}.get(name, 40.0)  # longer TTLs
        ...
        if news_score > 0.10 and news_direction:  # lower news threshold from 0.15
            components.append(SignalComponent(name="NEWS", direction=news_direction, raw_score=news_score,
                weighted_score=news_score * self.get_adjusted_weight("NEWS"), edge_contribution=news_score * 0.06, ttl_seconds=90.0))
...
class _FallbackNewsResult:
    def __init__(self, direction=None, strength=0.0):
        ...
        self.is_actionable = strength >= 0.20 and direction is not None  # lowered from 0.25
class NewsEngineShim:
 ...
 News engine mode: {self._mode}"
    ...
    def _fallback_eval(self, market, price_ctx):
        ...
        if strength < 0.12:  # lowered from 0.15
            return None
...
class SovereignPaperNewsSniper:
 ...
 Booting {VERSION}")
        ...
        logger.info(f"
 Boot | cash=${self.cash:.2f} | positions={len(self.positions)} | "
                     f"obi_buy={self.config.price_signal.obi_buy_threshold} | "
                     f"min_stacked_edge={self.config.edge_stacking.min_stacked_edge} | "
                     f"stop_loss={self.config.exit.stop_loss_pct} | "
                     f"max_hold={self.config.exit.max_hold_seconds}s")
    async def shutdown(self):
        if self.session: await self.session.close()
        self.db.close(); logger.info("
 Shutdown complete")
 ...
 Loaded {len(self.positions)} positions")
    ...
    def _compute_exec_quality(self, pc):
        ...
        sq = clamp01(1.0 - min(pc.spread / 0.10, 1.0))
        dq = clamp01(pc.total_depth / 300.0)
...
'
    ...
    async def fetch_markets(self):
        try:
            ...
            headers = {"Accept": "application/json", "User-Agent": "SovereignPaperTrader/10.1.2"}
            async def _do():
            ...
            for i in range(0, min(len(gm), 300), 25):
                batch = gm[i:i + 25]
            ...
            logger.info(f"
 FETCH | Gamma={len(gm)} enriched={len(enriched)}")
 ...
 fetch: {e}"); return []
    ...
    def _assess_book_quality(self, pc, relaxed=False):
        ...
        if mx > 0 and mn / mx < (0.03 if relaxed else 0.06): return False, 0.0, "imbalanced"
    ...
    async def hybrid_select_markets(self, markets):
        ...
        target = min(60, max(15, self.config.loop.market_discovery_min_rotation))
        ...
        if not dedup:
            logger.warning("
 FORCE TRADABLE | using ranked fallback")
            dedup = []; seen_fb = set()
            for m in ranked:
                mid = str(m.get("id", ""))
        ...
        logger.info(f"
 SELECTED | tradable={len(dedup[:60])} cache_hits={cache_hits} probe_limit={probe_limit}")
        return dedup[:60]
    ...
    def filter_and_rank_markets(self, markets):
        ...
        if rejected:
            ...
            logger.info(f"
 FILTER | input={len(markets)} passed={len(filtered)} output={len(out)} | rejects=[{', '.join(f'{k}={v}' for k, v in top_rejects)}]")
        return out
    ...
    async def scan_price_signals(self, markets):
        ...
        logger.info(f"
 SCAN | tokens={diag['tokens']} valid={diag['valid']} stacked={diag['stack_ok']}/{diag['stack_ok']+diag['stack_fail']} "
    ...
    def _emit_edge_report(self):
        ...
        try:
            ...
            logger.info(f"
 EDGE | trades={len(rows)} pnl=${tp:.2f} wr={w / len(rows) * 100:.1f}%")
            perf = self.edge_stacker.get_performance_report()
            if perf: logger.info(f"
 BANDIT | {json.dumps({k: v for k, v in perf.items() if v.get('trades', 0) >= 3})}")
            try:
                news_stats = self.news_engine.get_stats()
                logger.info(f"
 NEWS_ENGINE | {json.dumps(news_stats)}")
            except Exception:
                pass
            logger.info(f"
 EXEC_FEEDBACK | {json.dumps(self.exec_feedback.summary())}")
        except Exception: pass
            ...
            while self.running:
                ...
                try:
                    ...
                    if not opps:
                        ...
                        await asyncio.sleep(min(self.config.loop.loop_sleep * (1 + self._empty_cycles * 0.2), 30)); continue
                    ...
                    for opp in opps:
                        ...
                        analysis = TradeAnalysis(direction=stacked.direction, confidence=stacked.total_score,
                            ...
                            effective_sl_pct=self.config.exit.stop_loss_pct)
                    ...
                    slog.cycle_summary(self.cycle_count, {"cash": round(self.cash, 2), "equity": round(eq, 2),
                        ...
                        "trades_this": trades_this, "wr": round(wr, 0), "obi": self._cycle_signals_obi,
                        "mom": self._cycle_signals_mom, "revt": self._cycle_signals_revt,
                        "entry_attempts": self.metrics.entry_attempts,
                    ...
                    self._v28_record_exception("main_cycle", e); logger.exception(f"
 CYCLE ERROR | {e}")
                    await asyncio.sleep(self.config.loop.no_trade_sleep)
        finally: await self.shutdown()
def parse_args():
    import argparse
    p = argparse.ArgumentParser(description="Sovereign Paper News Sniper v10.1.2 ELITE TUNED")
...
def show_status(config):
 ...
 Open: {len(op)}/{config.risk.max_open_positions}")
        ...
        if total[0]: print(f"
 Trades: {total[0]} | PnL: ${total[1]:.2f} | WR: {wins/total[0]*100:.1f}%")
        print(f"{'='*60}\n")
    finally: db.close()
...
def export_trades(config, output_file):
 ...
 No trades"); return
        ...
        print(f"
 Exported {len(rows)} trades to {output_file}")
    finally: db.close()
...
def main():
    ...
    if args.reset:
        if input("
 Reset? [y/N]: ").lower() == 'y': reset_state(config)
        return
    if args.export: export_trades(config, args.export); return
    if args.health:
        try: db = DatabaseManager(config.db_path); db.connect(); db.execute("SELECT 1"); db.close(); print("
 HEALTHY"); exit(0)
