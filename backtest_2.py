...
# Try to import, fall back to standalone mode
try:
    ...
    from v10_elite import (
        PriceSignalEngine, PriceSignalConfig, EdgeStackingEngine,
        EdgeStackingConfig, ExecutionPriorityConfig, AdaptiveThresholdsConfig,
        FeedbackReinforcementConfig, PriceContext, PriceTick, EliteExitEngine,
        ExitConfig, Config, ExecutionEVConfig, TradabilityModel, ExecutionEVModel,
        safe_float, clamp, clamp01, detect_regime, TradeDirection, ExecutionTier,
        Position,
    )
    HAS_BOT = True
except ImportError:
    HAS_BOT = False
    print("⚠️  v10_elite.py not found — running in standalone mode (limited)")
...
# ═══════════════════════════════════════════════════════════════
# [FIX-5] SIMPLIFIED FILL SIMULATOR FOR BACKTEST
# ═══════════════════════════════════════════════════════════════

class BacktestFillSimulator:
    """
    Lightweight fill simulation matching the live PaperTradingSimulator's
    miss-probability logic without the full complexity.
    """

    def __init__(self, slippage_bps: float = 7.0):
    ...
    def simulate_entry(self, snap: BookSnapshot, direction: str,
        ...
        """
        Returns fill price or None if missed.
        Uses spread/depth-based miss probability similar to live bot.
        """
        self.attempts += 1
        pc = snap.to_price_context()

        # Compute relevant depth
        if direction == "YES":
            relevant_levels = pc.asks_raw[:5]
            top_size = safe_float(relevant_levels[0].get("size", 0)) if relevant_levels else 0
        else:
            relevant_levels = pc.bids_raw[:5]
            top_size = safe_float(relevant_levels[0].get("size", 0)) if relevant_levels else 0
        relevant_depth = sum(safe_float(l.get("size", 0)) for l in relevant_levels)

        # Miss probability (mirrors live PaperTradingSimulator logic)
        miss_prob = 0.03  # base
        if snap.spread > 0.08:
            miss_prob = max(miss_prob, 0.25)
        elif snap.spread > 0.05:
            miss_prob = max(miss_prob, 0.12)
        if top_size < 30:
            miss_prob = max(miss_prob, 0.15)
        elif relevant_depth < 100:
            miss_prob = max(miss_prob, 0.10)
        # Larger orders relative to depth miss more often
        if relevant_depth > 0 and size_usd > relevant_depth * 0.5:
            miss_prob = max(miss_prob, 0.30)

        if random.random() < miss_prob:
            self.misses += 1
            return None

        # [FIX-4] [FIX-6] Use executable side, not mid
        slip_mult = self.slippage_bps / 10000.0
        if direction == "YES":
            # Buy YES: pay the ask + slippage
            base = snap.ask
            fill = base + base * slip_mult
        else:
            # Buy NO: pay (1 - bid) + slippage
            base = 1.0 - snap.bid
            fill = base + base * slip_mult

        return max(0.01, min(0.99, fill))

    def simulate_exit(self, snap: BookSnapshot, direction: str) -> float:
        """
        [FIX-4] Exit at executable side with slippage.
        """
        slip_mult = self.slippage_bps * 0.7 / 10000.0  # exit slip slightly less
        if direction == "YES":
            # Sell YES: receive the bid - slippage
            base = snap.bid
            fill = base - base * slip_mult
        else:
            # Sell NO: receive (1 - ask) - slippage
            base = 1.0 - snap.ask
            fill = base - base * slip_mult
...
class BacktestEngine:
    ...
    def __init__(self, config=None, starting_cash: float = 1000.0,
        ...
        # [FIX-5] Fill simulator
        self.fill_sim = BacktestFillSimulator(slippage_bps)
    ...
    def replay(self, snapshots: List[BookSnapshot],
        ...
        print(f"  Fill simulation: ENABLED (miss prob based on spread/depth)")
    ...
    def _check_entry(self, tid: str, snap: BookSnapshot,
        ...
        if pc.spread > 0.03:
            ...
            if min(bd, ad) > 100:
                if bd > ad * 1.4:
                    s = min(1.0, (pc.spread - 0.03) / 0.06)
                    spread_sig = ("YES", round(s, 4), round(pc.spread * 0.25, 4))
                elif ad > bd * 1.4:
                    s = min(1.0, (pc.spread - 0.03) / 0.06)
                    spread_sig = ("NO", round(s, 4), round(pc.spread * 0.25, 4))
        ...
        fill_price = self.fill_sim.simulate_entry(snap, stacked.direction, size)
    ...
    def _check_exit(self, tid: str, snap: BookSnapshot):
        ...
        if exit_reason:
            ...
            exit_price = self.fill_sim.simulate_exit(snap, trade.direction)
